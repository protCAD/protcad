...
void writeStateToFile(protein* _prot, double _rad, double _phase, double _square, double _offset, double _coil,
	double _temp, double _energy, double _hismetaldist, double _zangle, double _yangle, double _HTdist, double _HTangle, string _fileName);
...
int main(int argc, char* argv[])
{
	...
	double offsetminhis, offsetmaxhis, offsetstep, phaseminhis, phasemaxhis, phasestep, radminhis, radmaxhis, radstep, coilminhis, coilmaxhis, radstep, squareminhis, squaremaxhis squarestep;
	...
	sscanf(parsedStrings[2].c_str(), "%lf", &coilstep);
	...
	double radhis = radminhis;
	double phasehis = phaseminhis;
	double squarehis = squareminhis;
	double offsethis = offsetminhis;
	double coilhis = coilminhis;
	...
	for (radhis = radhismin; radhis <= radhismax; radhis += radstep)
	{
	 for (phasehis = phasehismin; phasehis <= phasehismax; phasehis += phasestep)
	 {
	  for (offsethis = offsethismin; offsethis <= offsethismax; offsethis += offsetstep)
      {
	   for (squarehis = squarehismin; squarehis <= squarehismax; squarehis += squarestep)
	   {
	    for (coilhis = coilhismin; coilhis <= coilhismax; coilhis += coilstep)
		{	
			...
			undoBackbone(prot, radhis, phasehis, squarehis, offsethis, coilhis);
			writeStateToFile(prot, radhis, phasehis, squarehis, offsethis, coilhis, temp, bestEnergy, NE2FeDist, Zangle, Yangle, THdist, THangle,  "current.out");
		} //coil
	   }  //square
	  }   //offset
	 }    //phase
	...
	double _temp, double _energy,  double _hismetaldist, double _zangle, double _yangle, double _HTdist, double _HTangle, string _fileName)
{
	...
	file << " his metal " << _hismetaldist << " his por Z angle " << _zangle << " his por Y angle " << _yangle << " His Thr dist " << _HTdist << " His Thr angle " << _HTangle;
 	cout << " his metal " << _hismetaldist << " his por Z angle " << _zangle << " his por Y angle " << _yangle << " His Thr dist " << _HTdist
<< " His Thr angle " << _HTangle;
