...
double getEnergy(protein* _prot, vector <dblVec> _ligand);
...
double getProteinLigandEnergy(protein* _prot, vector <dblVec> _ligCoords, double _r0, double _eps);
...
int main (int argc, char* argv[])
{
	...
	string ligandFile = parsedStrings[1];
	...
	vector <dblVec> ligand = getLigandCoords(ligandFile);
	...
	for (UInt i = 0; i < iterations; i ++)
	{
		...
		if ( 0.80 > ranNumber.getNext()) // then deal with intrachain optimization
		{
			...
			if (choice == 0)
			{
				...
				double score = getEnergy(prot, ligand);
				if (score < oldScore)
				{
					oldScore = score;
...
double getEnergy(protein* _prot, vector <dblVec> _ligand)
{
	...
	double force = 0.0;
	if (radius > 6) force = - 3 * (6 - radius);
	double ligandEnergy = getProteinLigandEnergy(_prot, _ligand, 3.0, -0.1);
	double score = ligandEnergy + force + vdwE + 5.0*(float)nclashes - 5.0*(float)nhbs;
	//cout << "\tvdw " << vdwE << " ligand " << ligandEnergy << " force, radius " << force << " " << radius << " ";
	return score;
}
            ...
            for (UInt k = 0; k < _prot->getNumAtoms(i,j); k ++)
            {
               ...
               for (UInt l = 0; l < _ligCoords.size(); l ++)
               {
                   ...
                   double distratio = _r0/distance;
					if (distance < _r0) 
					{
						energy += 10 - (10+_eps) * (distance / _r0);
					}
					else
