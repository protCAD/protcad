...
c Initialize all the variables before starting
      ...
      if (numdistances.gt.maxnumdistances) then
        print *,'Error! exceeds memory allotment for pmf.'
        print *,'Change maxnumdistances in parameters.h'
        print *,'and recompile'
        goto 999
      endif
...
c----------------------------------------------------------
       ...
       call pdbread_no_biomt (filename,databasedir)
...
222   end do
     ...
     &  numchainused,numpdbnames
      ...
      call PMFmodule_vol_raw(pmfShellCounterHiRes,numatomtypes,
     &   numatomtypes,numdistances,PMFHiRes,pmfMindis,pmfMaxdis,
     &    pmfStepSize)
