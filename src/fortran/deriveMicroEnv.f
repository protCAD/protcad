...
c
c The random probabilites of drawing an A,B,C, or D are assumed
c to be the same across all atom types and coordination numbers.
c Any deviation this creates from what is actually seen, therefore
c contains information about the preference of an atomtype/coord
c number pair for a given environment type.
c 
c This version is used when actually scoring misfolds.


      totalSumAtomsCatCoord = 0.0
      do i1 = 1,numatomtypes
       do i2 = 1,maxNumCoord
        do i3 = 1,4
          probCatCoord(i1,i2,i3) = 
     +     numatomsInCategoryTotal(i3,2)/
     +     numenvTotal
        end do
       end do
      end do

      print *,'#Specific Type / Total# -> probCatCoord(n,x,y)'
      print *,'      A  ','    B  ','    C  ','    D  '
      do i=1,maxNumCoord
       print 19,i,(probCatCoord(follow1,i,j),j=1,4)
      end do
      print *,'-----------------------------'
      print *
...
c Used when scoring misfolds
     ...
     +     TheoreticalProbWord(i1,i2,i3) *
     +     numatomsInCategoryTotal(i1,1)
         TotalTheoreticalProbWord = TotalTheoreticalProbWord +
     +     TheoreticalProbWord(i1,i2,i3)
        end do
       end do
      end do
...
c----------------------------------------------------------
       ...
       call pdbread_no_biomt_reduced (filename,misfolddir)
       do i=1,maxat
       	 Bfac(i) = 0.0
       end do
          ...
          if (comboTypesComp(i3,i4,2).eq.atomData(i1,4)) then
           if (comboTypesComp(i3,i4,3).eq.atomData(i1,5)) then
            if (comboTypesComp(i3,i4,4).eq.atomData(i1,6)) then
             tempreal = Eword(atomData(i1,1),
     &                        atomData(i1,2)+1,i4)
             print 71,filename,'Best',tempreal,category(i1,1),i2,
          ...
          if (comboTypesComp(i3,i4,2).eq.atomData(i1,4)) then
           if (comboTypesComp(i3,i4,3).eq.atomData(i1,5)) then
            if (comboTypesComp(i3,i4,4).eq.atomData(i1,6)) then
             tempreal = Eword(atomData(i1,1),
     &                        atomData(i1,2)+1,i4)
             print 71,filename,'Worst',tempreal,category(i1,1),i2,
     &        Coordination(i1),res(i1),trueResnum(i1),chain(i1),
     &        atomname(i1)
c             Bfac(i1) =  (tempreal-BestEnergy)/
c     &           (WorstEnergy-BestEnergy) * 50
...
c Print out some info for debugging purposes
      ...
      pmfFile = 'lowResPMF.dat'
      call PMFmodule_LowRes(pmfShellCounter,numatomtypes,
     &   numatomtypesEnv,numdistances,PMFENERGY,pmfFile)

      pmfFile = 'hiResPMF.dat'
      call PMFmodule(pmfShellCounterHiRes,numatomtypes,
     &   numatomtypes,numdistances,PMFHiRes,pmfFile)
     ...
     &   i3 = 1,numdistances)
      ...
      call writePMF('PMF.dat',PMFHiRes,maxnumatomtypes,
     &   maxnumatomtypes,numdistances)
