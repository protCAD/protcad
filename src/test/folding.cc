...
//--Run optimizaiton loop till grand minimum-------------------------------------------------------------
	for (UInt a = 0; a<100; a++)
	{
		...
		{  
			...
			{
				//--folding directions
				coords1 = bundle->getCoords(0, 0, "CA");
				coords2 = bundle->getCoords(1, 0, "CA");
				coords3 = bundle->getCoords(2, 0, "CA");
				...
				direction = 1, randtype = rand() % 2, distance = rand() % 2;
				do 
				{
					deltaTheta = ((rand() % 3) -1);
				} while (deltaTheta == 0);
