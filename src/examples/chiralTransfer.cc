...
double calcCbeta(protein* _prot, UInt _ch1, UInt _ch2);
	...
	for (UInt i = 0; i < iterations; i ++)
	{
		...
		if ( 0.80 > ranNumber.getNext()) // then deal with intrachain optimization
		{
			...
			if (choice == 0)
			{
				...
				double dnewPhi = 60 - 120*ranNumber.getNext();
				double dnewPsi = 60 - 120*ranNumber.getNext();
				double newPhi = oldPhi + dnewPhi;
				double newPsi = oldPsi + dnewPsi;
				if (newPhi < -180) newPhi = newPhi + 360;
				if (newPhi > 180) newPhi = newPhi - 360;
				if (newPsi < -180) newPsi = newPsi + 360;
				if (newPsi > 180) newPsi = newPsi - 360;
...
double getEnergy(protein* _prot)
{
	...
	double vdwE = 0.5*_prot->intraEnergy(0)+ 0.1* _prot->intraEnergy(0,1); 
	double cbeta =  calcCbeta(_prot, 0, 1);
	dblVec centroid = _prot->getBackBoneCentroid(0);
	double radius = sqrt(CMath::dotProduct(centroid, centroid));

	double force = -1 * (3 - radius);

	double score = cbeta + force + vdwE + 5.0*(float)nclashes - 10.0*(float)nhbs;
	return score;
}
...
{
	double energy = 0.0;
	for (UInt i = 0; i < _prot->getNumResidues(_ch1); i ++)
	{
		dblVec cbeta1 = _prot->getCoords(_ch1, i , 4);
		for (UInt j = 0; j < _prot->getNumResidues(_ch2); j ++)
		{
			dblVec cbeta2 = _prot->getCoords(_ch2, j, 4);
			dblVec temp = cbeta1 - cbeta2;
			double distance = sqrt(CMath::dotProduct(temp,temp));
			double emin = 1.0;
			double radmin = 3.3;
			double vdWE = emin * ( pow(radmin/distance, 8) - 2.0 * pow(radmin/distance, 4) );
			energy += vdWE;
		}
	}
	return energy;
