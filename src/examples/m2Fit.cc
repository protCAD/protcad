...
int main (int argc, char* argv[])
{
	...
	PDBInterface* theTARGET = new PDBInterface(targetFile);
	ensemble* theEnsembleTarget = theTARGET->getEnsemblePointer();
	molecule* theMolTarget = theEnsembleTarget->getMoleculePointer(0);
	target = static_cast<protein*>(theMolTarget);
	...
	oneCA = target->getCoords(0,0,"CA");
	sevCA = target->getCoords(0,6,"CA");
	if (sevCA[2] > oneCA[2]) 
	{
		prot->rotate(Y_axis, 180.0);
	}

	pdbWriter (target, "target.pdb");
...
double getDistance()
{
	...
	for (UInt chprot = 0; chprot < prot->getNumChains(); chprot ++)
	{
		double bestDistance = 1E10;
		for (UInt chtar = 0; chtar < target->getNumChains(); chtar ++)
		{
			double tempdist = 0.0;
			for (UInt i = 0; i < prot->getNumResidues(0); i ++)
			{
				dblVec CAprot = prot->getCoords(chprot, i, "CA");
				dblVec CAtarg = target->getCoords(chtar, i, "CA");
				tempdist += CMath::distance(CAprot,CAtarg);
			} 
			if (tempdist < bestDistance) bestDistance = tempdist;
		}
		totalDistance += bestDistance;
	}
	double numres = (double)(prot->getNumChains() * prot->getNumResidues(0));
	return sqrt(totalDistance / numres);
}
