...
VecDblVec minimize(VecDblVec _backbone, ran _ranNumber); 
...
VecDblVec minimize(VecDblVec _backbone, ran _ranNumber)
{
	if (verbose) cout << "calling minimize" << endl;
	UInt size = _backbone.size();

    // read in length scales for each parameter
    DblVec lambda(size);
    for (UInt i=0; i < size; i ++)
    {
		double min = _backbone[i][1];
		double max = _backbone[i][2];
		double diff = 0.1*(max-min);
		double newVal = _backbone[i][0] - diff/2.0 + _ranNumber.getNext()*diff;
    ...
    for (UInt i = 0; i < (UInt)p.num_rows(); i ++)
    {
		...
		VecDblVec tmpbackbone(0);
		tmpbackbone.push_back(pRow);
	...
	VecDblVec finalBackbone(0);
	finalBackbone.push_back(pRow);
	return finalBackbone;
}
...
double getEnsembleScore()
{
	...
	for (UInt i = 0; i < seqData.size(); i ++)
	{
		if (seqEncoding[i] == "u")
		{
			...
			//prot->saveCurrentState();
			mapSequence((int)i);
			uE.push_back(GETENERGYWITHRESTRAINTS() - refEnergies[i]);
		}
		if (seqEncoding[i] == "f")
		{
			//prot->undoState();
			//prot->saveCurrentState();
			mapSequence((int)i);
			fE.push_back(GETENERGYWITHRESTRAINTS() - refEnergies[i]);
		}
	}
	
	double e = 2.71828182846;
	for (UInt i = 0; i < uE.size(); i ++)
	{
		double diff = uE[i]-wt;
		double thisPD = 1.0 / ( pow(e,bD*diff) + 1.0);
		pD += thisPD;
	}
	if (uE.size() !=  0) pD = pD / (double)uE.size();
	if (!(pD >= 0.0 && pD <= 1.0))
	{
	...
	{
		double diff = fE[i]-wt;
		double thisPS= 1.0 / ( pow(e,bS*diff) + 1.0);
		pS += thisPS;
	}
	if (fE.size() != 0) pS = pS / (double)fE.size();
	if (! (pS >= 0.0 && pS <= 1.0))
	{
		cout << "BAD PARAMS" << endl;
		return 0.0;
	...
	double disE = aD*log(pD + gD);
	double favE = aS*log(pS + gS);
...
void getSequence(string _infile, VecUIntVec &_symmetryData)
{
	...
	while(getline(inFile, currentLine, '\n'))
	{
		...
		if (parsedStrings[0] == "w" || parsedStrings[0] == "f" || parsedStrings[0] == "u" || parsedStrings[0] == ".")
		{
